...
server_socket.bind(("192.168.56.1", 1234))  # Server IP and port
...
# Create a PyQt5 window for the server
class ServerWindow(QMainWindow):
    ...
    def generate_pair_code(self):
        # Generate a random 4-character pair code
    ...
    def send_message(self):
        ...
        if message:
            ...
            conn.send(encrypted_message)  # Send the message via the 'conn' socket
            self.text_browser.append(f'You: {message}')
            self.input_box.clear()

    def open_emoji_picker(self):
        emoji_picker = EmojiPicker()
        result = emoji_picker.exec_()
        if result == QDialog.Accepted:
            self.selected_emoji = emoji_picker.selected_emoji
            if self.selected_emoji:
                self.input_box.insert(self.selected_emoji)
